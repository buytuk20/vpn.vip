...
if (!process.env.JWT_SECRET) {
    console.warn('⚠️  تحذير أمني: JWT_SECRET غير معيّن — يُستخدم سرٌّ افتراضيٌّ ضعيف. عيّنه قبل الإنتاج!');
}
    ...
    return (req, res, next) => {
        ...
        if (entry.count > maxRequests) {
            return res.status(429).json({
                success: false,
                message: '🚫 محاولات كثيرة، انتظر دقيقةً ثم أعد المحاولة'
            });
        }
        next();
    };
    ...
    try {
        ...
        if (!(await adminRef.get()).exists) {
            ...
            await db.collection('users').doc('reseller1').set({
                username: 'reseller1',
                password: bcrypt.hashSync(RESELLER_PASS, 10),
                role: 'reseller',
                credits: 500,
                parent_id: 'admin',
        ...
        for (const s of REAL_SERVERS) {
            const snap = await db.collection('servers')
                .where('host', '==', s.host)
                .where('sni_hostname', '==', s.sni_hostname)
                .get();
            if (snap.empty) {
...
app.post('/api/login', rateLimit('login', 10, 60000), async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: { token, user: { id: doc.id, username: u.username, role: u.role, credits: u.credits } }
        });
    } catch (e) {
...
app.post('/api/user/login', rateLimit('userlogin', 8, 60000), async (req, res) => {
    try {
        ...
        if (a.status === 'frozen') {
            return res.json({
                success: false,
                message: '🔒 الحساب مجمّد' + (a.frozen_reason ? ': ' + a.frozen_reason : '') + '. تواصل مع الموزّع.'
            });
        }
        ...
        res.json({
            ...
            data: {
                ...
                account: {
                    ...
                    activated: a.activated || 0,
                    status: a.status || 'pending',
                    bandwidth_mode: a.bandwidth_mode || 'unlimited',
                    data_cap_mb: a.data_cap_mb || 0,
                    data_used_mb: Math.round(a.data_used_mb || 0),
        ...
        if (req.user.role === 'user') {
            ...
            return res.json({
                ...
                data: {
                    ...
                    bandwidth_mode: a.bandwidth_mode || 'unlimited',
                    data_cap_mb: a.data_cap_mb || 0,
                    session_hours: Math.round((a.session_hours || 0) * 10) / 10,
                    total_sessions: a.total_sessions || 0,
                    referral_code: a.referral_code || null,
                    referral_bonus_days: a.referral_bonus_days || 0,
                    allow_hotspot: a.allow_hotspot || 0,
                    max_hotspot_devices: a.max_hotspot_devices || 0,
                    hotspot_active: a.hotspot_active || 0,
    ...
    try {
        ...
        if (a.activated === 1 || a.status === 'active') {
            if (a.bound_device && a.bound_device !== fingerprint) {
                return res.json({
                    success: false,
                    message: '⚠️ هذا الحساب مربوطٌ بجهازٍ آخر. تواصل مع الموزّع لفصله إن لزم.'
                });
            }
        ...
        await ref.update({
            activated: 1,
            activated_at: TS.serverTimestamp(),
            bound_device: fingerprint,
...
app.get('/api/stats', authenticateToken, staff, async (req, res) => {
    try {
        ...
        let total = 0, active = 0, expired = 0, frozen = 0, pending = 0, exhausted = 0;
        const now = new Date();
...
app.post('/api/account/create', authenticateToken, staff, async (req, res) => {
    try {
        ...
        await db.collection('accounts').add({
            ...
            status: 'pending',
            bandwidth_mode: mode,
            data_cap_mb: mode === 'capped' ? (Number(data_cap_mb) || 0) : 0,
            data_used_mb: 0,
            session_hours: 0,
            total_sessions: 0,
            referral_code,
            allow_hotspot: 0,
            max_hotspot_devices: 0,
            hotspot_active: 0,
            created_at: TS.serverTimestamp()
        });
        ...
        res.json({
            success: true,
            message: 'تم الإصدار (سينشط عند أول استخدام)',
            data: {
                username: finalUsername,
...
app.post('/api/accounts/update', authenticateToken, staff, async (req, res) => {
    try {
        ...
        const a = cur.data();
        if (req.user.role === 'reseller' && a.reseller_id !== req.user.id) {
            return res.json({ success: false, message: 'هذا الحساب لا يخصّك' });
        }

        const upd = {};
        const notes = [];

        if (status && ['active', 'frozen', 'blocked', 'expired', 'pending', 'exhausted'].includes(status)) {
            upd.status = status;
            notes.push('الحالة ← ' + status);
            if (status === 'frozen') upd.frozen_reason = frozen_reason || 'مجمّد من الإدارة';
            if (status === 'active') upd.frozen_reason = TS.delete();
        }
        ...
        if (new_password) {
            upd.password = bcrypt.hashSync(String(new_password), 10);
            notes.push('إعادة ضبط كلمة السر');
        }
        ...
        if (bandwidth_mode) {
            upd.bandwidth_mode = bandwidth_mode === 'capped' ? 'capped' : 'unlimited';
            notes.push('البيانات ← ' + upd.bandwidth_mode);
        }
        if (data_cap_mb !== undefined) upd.data_cap_mb = Number(data_cap_mb) || 0;

        if (unbind) {
            upd.bound_device = TS.delete();
            upd.device_name = TS.delete();
            notes.push('فُصل الجهاز المرتبط');
        }
...
app.get('/api/plans', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({ success: true, plans: snap.docs.map(d => ({ id: d.id, ...d.data() })) });
    } catch (e) {
        res.json({ success: false, message: 'خطأ: ' + e.message });
    }
});
...
app.get('/api/servers', authenticateToken, async (req, res) => {
    try {
        const snap = await db.collection('servers').get();
        res.json({ success: true, servers: snap.docs.map(d => ({ id: d.id, ...d.data() })) });
    } catch (e) {
        res.json({ success: false, message: 'خطأ: ' + e.message });
    }
});
...
app.get('/api/servers/:id/v2config', authenticateToken, async (req, res) => {
    try {
        ...
        const s = doc.data();
        if (s.is_active === 0 && req.user.role === 'user') {
            return res.json({ success: false, message: 'السيرفر غير متاح' });
        }

        const config = buildV2Config(s);
        res.json({
            success: true,
            engine: s.engine || 'v2ray',
            config,
            configString: JSON.stringify(config)
        });
    } catch (e) {
...
app.post('/api/servers/save', authenticateToken, onlyAdmin, async (req, res) => {
    try {
        const b = req.body;
        if (!b.display_name || !b.host || !b.sni_hostname) {
            return res.json({ success: false, message: 'الاسم والمضيف و SNI مطلوبة' });
        }
        ...
        const data = {
            ...
            protocol: String(b.protocol || 'vless'),
            uuid: String(b.uuid || '').trim(),
            network: String(b.network || 'tcp'),
            security: String(b.security || 'reality'),
        ...
        if (b.id) await db.collection('servers').doc(b.id).set(data, { merge: true });
...
app.delete('/api/servers/:id', authenticateToken, onlyAdmin, async (req, res) => {
    try {
        await db.collection('servers').doc(req.params.id).delete();
        res.json({ success: true, message: 'تم حذف السيرفر' });
    } catch (e) {
        res.json({ success: false, message: 'خطأ: ' + e.message });
    }
});
...
app.delete('/api/users/:id', authenticateToken, onlyAdmin, async (req, res) => {
    try {
        if (req.params.id === req.user.id) {
            return res.json({ success: false, message: 'لا يمكنك حذف نفسك' });
        }
...
app.get('/api/hotspot/:accountId', authenticateToken, staff, async (req, res) => {
    try {
        if (req.user.role === 'reseller') {
            const acc = await db.collection('accounts').doc(req.params.accountId).get();
            if (!acc.exists || acc.data().reseller_id !== req.user.id) {
                return res.json({ success: false, message: 'هذا الحساب لا يخصّك' });
            }
        }

        const snap = await db.collection('hotspot_devices')
...
app.post('/api/hotspot/link', authenticateToken, staff, async (req, res) => {
    try {
        ...
        const newRef = await db.collection('accounts').add({
            reseller_id: req.user.id,
            username: finalUsername,
            password: bcrypt.hashSync(finalPassword, 10),
            plan_id,
        ...
        await db.collection('hotspot_devices').doc(device_id).update({
            linked_account_id: newRef.id,
            is_approved: 1
        });
...
app.delete('/api/hotspot/:id', authenticateToken, staff, async (req, res) => {
    try {
        await db.collection('hotspot_devices').doc(req.params.id).delete();
...
app.post('/api/hotspot/report', authenticateToken, async (req, res) => {
    try {
        ...
        if (!acc.exists || !acc.data().allow_hotspot) {
            return res.json({ success: false, message: 'الحساب لا يدعم Hotspot' });
        }
        ...
        if (Array.isArray(devices)) {
            for (const d of devices.slice(0, max)) {
                const q = await db.collection('hotspot_devices')
                ...
                const payload = {
                    ...
                    last_seen: TS.serverTimestamp()
                };

                if (q.empty) await db.collection('hotspot_devices').add(payload);
